...
function gameTime() {
    _gameTimeNum--;
    if (_gameTimeNum <= 0) {
        GameTimeLayer.innerHTML = '&nbsp;&nbsp;&nbsp;&nbsp;时间到！不能吃喽～';
        gameOver();
...
function showGameScoreLayer() {
    ...
    let score_text = '你吃了 ';
    score_text += "<span style='color:red;'>" + (deviation_time / 1000).toFixed(2) + "</span>" + ' 秒哦！<br>您吃掉的染染数为 ';
    score_text += "<span style='color:red;'>" + _gameScore + "</span>";
    score_text += '<br>您平均每秒吃掉了 ';
    ...
    document.getElementById('GameScoreLayer-bast').innerHTML = '历史最佳食量 ' + "<span style='color:red;'>" + bast + "</span>";
...
function shareText(score) {
    ...
    if (score <= 0 * __Time) return '杂鱼！你一个也没吃！染染都跑啦！';
     if (score <= 2.5 * __Time) return '快点呐！还不够！';
    if (score <= 5 * __Time) return '你真的认真了吗？';
    if (score <= 7.5 * __Time) return '哎呦我 不行了吧，杂鱼～';
    if (score <= 10 * __Time) return '啊？！';
    return '666这个入开桂了';
}
